use crate::{
    analyzer::{dependency_parser::Language, tool_management::ToolInstaller},
    cli::{OutputFormat, ToolsCommand},
};
...
pub async fn handle_tools(command: ToolsCommand) -> crate::Result<()> {
    match command {
        ...
        ToolsCommand::Verify {
            languages,
            detailed,
        } => handle_tools_verify(languages, detailed),
        ToolsCommand::Guide {
            languages,
            platform,
        } => handle_tools_guide(languages, platform),
    }
}
...
fn get_tool_for_language<'a>(
    installer: &mut ToolInstaller,
    language: &Language,
) -> (&'a str, bool) {
    match language {
        Language::Rust => (
    ...
    match language {
        ...
        Language::Java => {
            ...
            match target_platform {
                ...
                "Linux" => {
                    println!("    Install: Download from https://github.com/anchore/grype/releases")
                }
                _ => {
                    println!("    Install: Download from https://github.com/anchore/grype/releases")
                }
