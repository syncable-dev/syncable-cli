...
pub async fn handle_dependencies(
    ...
    if format == OutputFormat::Table {
        let table_output = display_dependencies_table(
            &dep_analysis,
            &monorepo_analysis,
            licenses,
            vulnerabilities,
...
async fn display_dependencies_table(
    ...
    for (name, info) in &dep_analysis.dependencies {
        ...
        // Color code by type
        stdout.set_color(ColorSpec::new().set_fg(Some(if info.is_dev {
            Color::Yellow
        } else {
            Color::Green
        })))?;
        ...
        if licenses && info.license.is_some() {
            let license_info = format!(
                " - License: {}",
                info.license.as_ref().unwrap_or(&"Unknown".to_string())
            );
            print!("{}", license_info);
...
fn display_license_summary(
    dependencies: &analyzer::dependency_parser::DetailedDependencyMap,
) -> String {
    let mut output = String::new();
...
async fn check_and_display_vulnerabilities(
    ...
    let mut deps_by_language: HashMap<
        analyzer::dependency_parser::Language,
        Vec<analyzer::dependency_parser::DependencyInfo>,
    > = HashMap::new();
    ...
    match checker
        .check_all_dependencies(&deps_by_language, project_path)
        .await
    {
        Ok(report) => {
            let mut stdout = StandardStream::stdout(ColorChoice::Always);
...
fn display_vulnerable_dependencies(
    ...
    for vuln_dep in &report.vulnerable_dependencies {
        ...
        for vuln in &vuln_dep.vulnerabilities {
            ...
            stdout.set_color(
                ColorSpec::new()
                    .set_fg(Some(match vuln.severity {
