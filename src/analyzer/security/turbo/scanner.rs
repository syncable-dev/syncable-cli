...
use crate::analyzer::security::{SecurityCategory, SecurityFinding, SecuritySeverity};
...
impl FileScanner {
    ...
    fn scan_file(&self, task: ScanTask) -> ScanResult {
        ...
        let matches = self
            .pattern_engine
            .scan_content(&content, task.quick_reject, &task.file);
    ...
    fn is_code_generation_file(&self, content: &str, file_meta: &FileMetadata) -> bool {
        ...
        let pattern_matches = code_gen_content_patterns
            .iter()
    ...
    fn convert_matches_to_findings(
        &self,
        ...
        matches
            ...
            .map(|match_| SecurityFinding {
                ...
                severity: self.adjust_severity(
                    &match_.pattern.severity,
                    file_meta,
                    match_.confidence,
                ),
        ...
        if file_meta.is_gitignored {
            ...
            if self.file_contains_secrets(file_meta) {
                ...
                if self.is_file_tracked_by_git(&file_meta.path) {
                    ...
                    description
                        .push_str(" (File is NOT in .gitignore but contains secrets - HIGH RISK)");
                }
            } else {
                description.push_str(" (File appears safe for version control)");
            }
    ...
    fn adjust_severity(
        ...
        if !file_meta.is_gitignored
            && matches!(severity, SecuritySeverity::Medium | SecuritySeverity::High)
        {
    ...
    fn get_compliance_frameworks(&self, category: &SecurityCategory) -> Vec<String> {
        match category {
            SecurityCategory::SecretsExposure => vec![
                "SOC2".to_string(),
                "GDPR".to_string(),
                "PCI-DSS".to_string(),
            ],
            SecurityCategory::InsecureConfiguration => {
                vec!["SOC2".to_string(), "OWASP".to_string()]
            }
            SecurityCategory::AuthenticationSecurity => {
                vec!["SOC2".to_string(), "OWASP".to_string()]
            }
            ...
            if let Some(eq_pos) = line.find('=') {
                ...
                if is_sensitive_env_key(key) && !value.is_empty() && !is_placeholder_value(value) {
                    findings.push(SecurityFinding {
                        ...
                        description: format!(
                            "Environment variable '{}' contains a potentially sensitive value",
                            key
                        ),
                        ...
                        references: vec!["https://12factor.net/config".to_string()],
...
fn determine_env_severity(key: &str, _value: &str) -> SecuritySeverity {
    ...
    if key_upper.contains("DATABASE")
        || key_upper.contains("DB_PASS")
        || key_upper.contains("AWS_SECRET")
        || key_upper.contains("STRIPE_SECRET")
    {
        return SecuritySeverity::Critical;
    }

    // High: Most API keys and secrets
    if key_upper.contains("API")
