...
mod tests {
    ...
    #[test]
    fn test_node_azure_functions_detection() {
        let language = DetectedLanguage {
            name: "JavaScript".to_string(),
            version: Some("18.0.0".to_string()),
            confidence: 0.95,
            files: vec![
                PathBuf::from("api/HttpTrigger1/index.ts"),
                PathBuf::from("host.json"),
                PathBuf::from("api/HttpTrigger1/function.json"),
            ],
            main_dependencies: vec!["@azure/functions".to_string()],
        ...
        // Should detect Azure Functions as a primary backend framework
        let azure_functions = technologies.iter().find(|t| t.name == "Azure Functions");

        if let Some(af) = azure_functions {
            assert!(matches!(af.category, TechnologyCategory::BackendFramework));
            assert!(af.is_primary);
