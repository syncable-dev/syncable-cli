...
#[test]
fn nextjs_detects_with_exact_app_folder() {
    let language = DetectedLanguage {
        name: "TypeScript".to_string(),
        version: Some("5.0.0".to_string()),
        confidence: 0.95,
        files: vec![
            std::path::PathBuf::from("src/app/page.tsx"),
            std::path::PathBuf::from("src/app/layout.tsx"),
        ],
        main_dependencies: vec![],
        dev_dependencies: vec![],
        package_manager: Some("npm".to_string()),
    ...
    assert!(
        technologies.iter().any(|t| t.name == "Next.js"),
        "Expected Next.js to be detected when using 'app' folder"
    );
}

#[test]
fn nextjs_does_not_detect_with_application_folder() {
    let language = DetectedLanguage {
        name: "TypeScript".to_string(),
        version: Some("5.0.0".to_string()),
        confidence: 0.95,
        files: vec![
            std::path::PathBuf::from("src/application/page.tsx"),
            std::path::PathBuf::from("src/application/layout.tsx"),
        ],
        main_dependencies: vec![],
        dev_dependencies: vec![],
        package_manager: Some("npm".to_string()),
    ...
    let technologies = detect_frameworks(project_root, &[language], &config).unwrap();
    // Should not detect by structure because 'application' must not match 'app'
    assert!(
        !technologies.iter().any(|t| t.name == "Next.js"),
        "Should not detect Next.js when folder is 'application'"
    );
}
