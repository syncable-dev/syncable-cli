...
repliesRouter.post('/generate', async (c) => {
    try {
        ...
        const conversationId = dbOps.createConversation(message.trim(), tone, context?.trim() || undefined, intent?.trim() || undefined);
 ...
 */
function buildUserMessage(message, context, intent, enhancedContext) {
    let parts = [];
