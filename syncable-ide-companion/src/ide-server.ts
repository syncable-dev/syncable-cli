/**
 * Syncable IDE Companion - MCP Server
 */
...
const IDE_SERVER_PORT_ENV_VAR = 'SYNCABLE_CLI_IDE_SERVER_PORT';
const IDE_WORKSPACE_PATH_ENV_VAR = 'SYNCABLE_CLI_IDE_WORKSPACE_PATH';
const IDE_AUTH_TOKEN_ENV_VAR = 'SYNCABLE_CLI_IDE_AUTH_TOKEN';
...
function sendIdeContextUpdateNotification(
  ...
  transport.send(notification).catch((err) => {
    log(`Failed to send context update: ${err}`);
  });
}
    ...
    return new Promise((resolve) => {
      ...
      const onDidChangeDiffSubscription = this.diffManager.onDidChange((notification) => {
        for (const transport of Object.values(this.transports)) {
          transport.send(notification).catch((err) => {
            this.log(`Failed to send diff notification: ${err}`);
          });
        }
      });
        ...
        if (sessionId && this.transports[sessionId]) {
          ...
          const keepAlive = setInterval(() => {
            ...
            transport
              ...
              .catch((error) => {
                missedPings++;
                this.log(`Failed to send keep-alive ping for session ${sid}. Missed: ${missedPings}`);
                if (missedPings >= 3) {
                  this.log(`Session ${sid} missed ${missedPings} pings. Closing.`);
          ...
          mcpServer.connect(transport).catch((err) => {
            this.log(`Failed to connect MCP server: ${err}`);
          });
        } else {
      ...
      this.server = app.listen(0, '127.0.0.1', async () => {
        ...
        if (address && typeof address !== 'string') {
          ...
          try {
            const portDir = path.join(os.tmpdir(), 'syncable', 'ide');
            await fs.mkdir(portDir, { recursive: true });
            portFile = path.join(portDir, `syncable-ide-server-${process.ppid}-${this.port}.json`);
...
const createMcpServer = (diffManager: DiffManager, log: (message: string) => void) => {
  const server = new McpServer(
    {
      name: 'syncable-ide-companion-mcp-server',
      version: '1.0.0',
    },
